...
async function handleSubmit() {
    try {
        ...
        // Get current values directly from textareas
        const getTextAreaContent = (containerId) => {
            const container = document.querySelector(`.text-box-container.${containerId}`);
            const textarea = container?.querySelector('.text-area');
            console.log(`Getting content for ${containerId}:`, textarea?.value);
        ...
        const formData = {
            r3Form: {
                ...
                asc: ascStatus,
                adhd: adhdStatus,
                observations: getTextAreaContent('clinical'),      // maps to 'clinical' class
                strengths: getTextAreaContent('strengths'),        // maps to 'strengths' class
                supportareas: getTextAreaContent('priority'),      // maps to 'priority' class
                recommendations: getTextAreaContent('support'),    // maps to 'support' class
...
function clearForm() {
    ...
    document.querySelectorAll('.text-area').forEach(textarea => {
        textarea.value = textarea.closest('.text-box-container')?.dataset.defaultText || '';
    });
