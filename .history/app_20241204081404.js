...
function initializeExpandableFields() {
    document.querySelectorAll('.expandable-field').forEach(field => {
        ...
        if (preview) {
            ...
            // Initialize field content if empty
            ...
            preview.addEventListener('click', () => {
                ...
                textarea.dataset.fieldId = fieldId; // Store field ID in textarea
                ...
                // Handle input for live sync
                const inputHandler = debounce(() => {
                    const content = textarea.value.trim();
                    const currentFieldId = textarea.dataset.fieldId;
                    window.fieldContents[currentFieldId] = content;
                    preview.textContent = content || fieldConfig[currentFieldId].defaultText;
                    saveToLocalStorage();
                }, 300);
                ...
                newTextarea.addEventListener('input', inputHandler);
                newTextarea.focus();
...
function clearForm() {
    ...
    currentRowId = null; // Reset the stored row ID
