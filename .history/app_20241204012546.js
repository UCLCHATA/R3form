...
async function handleSubmit() {
    try {
        ...
        if (!existingData || (!existingData.r3Form && !existingData.r3Forms)) {
            console.error('Unexpected API response structure:', existingData);
            throw new Error('Invalid API response format');
        }

        // Try both singular and plural forms
        const entries = existingData.r3Forms || existingData.r3Form || [];
        console.log('Parsed entries:', entries);

        // Find existing entry by CHATA_ID
        const existingEntry = Array.isArray(entries) ? 
            entries.find(entry => entry.CHATA_ID === selectedChataId) : 
            (entries.CHATA_ID === selectedChataId ? entries : null);
