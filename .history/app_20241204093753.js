...
async function checkExistingSubmission(chataId) {
    try {
        ...
        if (data && data.r3Form) {
            ...
            const existingRow = data.r3Form.find(row => row.chata_id === chataId);
            
            if (existingRow) {
                console.log('Found existing submission:', existingRow);
                removeExistingWarnings(); // Clear any existing warnings first
                displaySubmissionWarning(existingRow);
                return existingRow.id; // Return the row ID
            }
        }
...
async function handleSubmit() {
    try {
        ...
        let response;
        if (currentRowId) {
            console.log(`Updating row ${currentRowId} with data:`, JSON.stringify(formData, null, 2));
            response = await updateRow(currentRowId, formData);
        } else {
