...
async function checkExistingSubmission(chataId) {
    try {
        ...
        if (cachedFormData) {
            console.log('Checking cache for existing submission');
            existingSubmission = cachedFormData.find(form => form.chataId === chataId);
        }
        
        // If not in cache, check API using filter
        if (!existingSubmission) {
            console.log('Not found in cache, checking API');
            const response = await fetch(`${R3_FORM_API}?filter[chataId]=${chataId}`);
            if (response.ok) {
                const data = await response.json();
                if (data && data.r3Form && data.r3Form.length > 0) {
                    existingSubmission = data.r3Form[0]; // Get the first matching submission
        ...
        if (existingSubmission) {
            ...
            displaySubmissionWarning(existingSubmission);
            return existingSubmission; // Return the full submission object
        }
...
async function handleSubmit() {
    try {
        ...
        // Check for existing submission
        const existingSubmission = await checkExistingSubmission(selectedChataId);
        if (existingSubmission) {
            const proceed = confirm('An existing assessment was found for this CHATA ID. Submitting will replace the previous assessment. Do you want to continue?');
            if (!proceed) {
                return;
            }
        }
        ...
        let response;
        if (existingSubmission) {
            // Update existing submission using PUT request with row number
            console.log(`Updating existing submission with row ID: ${existingSubmission.id}`);
            response = await fetch(`${R3_FORM_API}/${existingSubmission.id}`, {
                method: 'PUT',
                headers: {
            ...
            // Create new submission using POST request
            console.log('Creating new submission');
            response = await fetch(R3_FORM_API, {
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json'
        ...
        const cachedFormData = getCachedData(CACHE_KEYS.FORM_DATA) || [];
        const updatedFormData = existingSubmission 
            ? cachedFormData.map(form => form.id === existingSubmission.id ? formData.r3Form : form)
            : [...cachedFormData, formData.r3Form];
        setCacheData(CACHE_KEYS.FORM_DATA, updatedFormData);
