...
async function checkExistingSubmission(chataId) {
    try {
        ...
        console.log('Existing form data:', data);
        
        if (data && data.r3Form) {
            // Find ALL rows with matching CHATA_ID
            const existingRows = data.r3Form.filter(row => row.chata_id === chataId);
            
            if (existingRows.length > 0) {
                console.log('Found existing submissions:', existingRows);
                removeExistingWarnings(); // Clear any existing warnings first
                
                // Use the first row's ID for updating
                const rowToUpdate = existingRows[0];
                displaySubmissionWarning(rowToUpdate);
                
                // If there are multiple rows, we'll update the first one
                if (existingRows.length > 1) {
                    console.log(`Found ${existingRows.length} rows with CHATA_ID ${chataId}. Will update row ${rowToUpdate.id}`);
                }
...
// API Functions
async function updateRow(rowId, formData) {
    console.log(`Making PUT request to ${R3_FORM_API}/${rowId} with data:`, formData);
    const response = await fetch(`${R3_FORM_API}/${rowId}`, {
        method: 'PUT',
        headers: {
...
async function createRow(formData) {
    console.log(`Making POST request to ${R3_FORM_API} with data:`, formData);
    const response = await fetch(R3_FORM_API, {
        method: 'POST',
        headers: {
            'Accept': 'application/json',
...
async function handleSubmit() {
    try {
        ...
        console.log('Selected CHATA ID:', selectedChataId);
        
        const selectedData = chataData.find(item => item.id === selectedChataId);
        console.log('Selected CHATA data:', selectedData);
        ...
        const existingRowId = await checkExistingSubmission(selectedChataId);
        console.log('Existing row ID:', existingRowId);
        ...
        // Get referrals data
        const referrals = window.fieldContents.referrals ?
        ...
        // Log current field contents for debugging
        console.log('Current field contents:', window.fieldContents);
        ...
        const formData = {
            r3Form: {
                ...
                asc: ascStatus,
                adhd: adhdStatus,
                observations: window.fieldContents['clinical-observations'] || '',
                strengths: window.fieldContents['strengths-abilities'] || '',
                supportareas: window.fieldContents['priority-support'] || '',
                recommendations: window.fieldContents['support-recommendations'] || '',
                referrals: referrals
            }
        };
        ...
        let response;
        // If we found an existing row, always update it
        if (existingRowId) {
            console.log(`Updating existing row ${existingRowId} with data:`, JSON.stringify(formData, null, 2));
            response = await updateRow(existingRowId, formData);
        } else {
            console.log('Creating new row with data:', JSON.stringify(formData, null, 2));
            response = await createRow(formData);
        }

        const result = await response.json();
        console.log('API Response:', result);

        alert(existingRowId ? `Row ${existingRowId} updated successfully!` : 'New row created successfully!');
