// Replace FORM_RESPONSES_API with your Google Apps Script Web App URL after deployment
const SCRIPT_URL = 'YOUR_SCRIPT_URL_HERE';

// Keep all existing code unchanged, only modify these functions:
...
async function loadChataData() {
    try {
        ...
        if (result.result === 'success') {
            ...
            console.log('Raw CHATA data:', data);
            
            // Map the data to the format we need
            chataData = data
                .filter(row => {
                    const id = row.CHATA_ID;
                    const name = row.Name;
