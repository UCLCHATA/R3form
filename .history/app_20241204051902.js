...
function setupModalResize(modal) {
    ...
    modal.addEventListener('mousedown', (e) => {
        ...
        const isClickInResizeZone = (e.clientX > rect.right - 15 && e.clientY > rect.bottom - 15);
        
        if (isClickInResizeZone) {
            isResizing = true;
            startX = e.clientX;
            startY = e.clientY;
    ...
    document.addEventListener('mousemove', (e) => {
        ...
        // Calculate available space in form area
        const formArea = document.querySelector('.form-content');
        const formRect = formArea.getBoundingClientRect();
        const maxWidth = formRect.width - 40; // Account for padding
        
        const width = startWidth + (e.clientX - startX);
        const height = startHeight + (e.clientY - startY);
        const maxHeight = window.innerHeight * 0.8;

        // Constrain width to form area
        modal.style.width = `${Math.min(Math.max(width, 300), maxWidth)}px`;
        modal.style.height = `${Math.min(Math.max(height, 200), maxHeight)}px`;
    });
    ...
    if (clearButton) {
        clearButton.addEventListener('click', () => {
            if (confirm('Are you sure you want to clear the form? This action cannot be undone.')) {
                clearForm();
                // Also clear the CHATA ID selection
                if (chataIdSelect) {
                    chataIdSelect.value = '';
                }
                // Clear any warnings
    ...
    if (refreshButton) {
        refreshButton.addEventListener('click', async () => {
            try {
                ...
                populateChataDropdown(); // Repopulate dropdown after refresh
...
function handleExpandableTextResize(textarea) {
    ...
    if (modalContainer) {
        ...
        const newHeight = Math.min(contentHeight + headerHeight + padding, maxHeight);
        
        // Get form area width
        const formArea = document.querySelector('.form-content');
        if (formArea) {
            const formWidth = formArea.getBoundingClientRect().width;
            const newWidth = formWidth * 0.95;
            
            // Update both modal and textarea
            modalContainer.style.height = `${newHeight}px`;
            modalContainer.style.width = `${newWidth}px`;
            modalContainer.style.right = '20px'; // Keep modal on right side
            
            // Update textarea size
            textarea.style.height = `${newHeight - headerHeight - padding}px`;
