...
async function checkExistingSubmission(chataId) {
    ...
    try {
        ...
        if (cachedFormData) {
            ...
            console.log('Cache check result:', existingSubmission ? 'Found in cache' : 'Not found in cache');
        }

        // If not in cache, check API
        if (!existingSubmission) {
            console.log('Checking API for existing submission');
            ...
            if (response.ok) {
                ...
                if (data && data.r3Form) {
                    ...
                    if (existingSubmission) {
                        // Update cache with the found submission
                        const updatedCache = cachedFormData || [];
                        if (!updatedCache.find(form => form.chataId === chataId)) {
                            updatedCache.push(existingSubmission);
                            setCacheData(CACHE_KEYS.FORM_DATA, updatedCache);
                        }
                    }
                    console.log('API check result:', existingSubmission ? 'Found in API' : 'Not found in API');
                }
            }
        }
        ...
        if (existingSubmission) {
            // Add warning to the header
            addHeaderWarning();
            // Add warning to the submit button
            addSubmitButtonWarning();
            return true;
        }
...
// Function to add header warning
function addHeaderWarning() {
    const warning = document.createElement('div');
    warning.className = 'existing-submission-warning';
    warning.innerHTML = `
        <i class="material-icons">warning</i>
        <span>An existing assessment was found for this CHATA ID</span>
    `;
...
// Function to add submit button warning
function addSubmitButtonWarning() {
    const warning = document.createElement('div');
    warning.className = 'submit-warning';
    warning.innerHTML = `
        <i class="material-icons">info</i>
        <span>Submitting will replace all previous assessments for this CHATA ID</span>
    `;
    const buttonGroup = document.querySelector('.button-group');
    if (buttonGroup && !buttonGroup.querySelector('.submit-warning')) {
        buttonGroup.insertBefore(warning, buttonGroup.firstChild);
    }
}
...
async function handleSubmit() {
    try {
        ...
        // Check for existing submission
        const hasExisting = await checkExistingSubmission(selectedChataId);
        if (hasExisting && !confirm('An existing assessment was found for this CHATA ID. Submitting will replace all previous assessments for this CHATA ID. Do you want to continue?')) {
            return;
        }
        ...
        // Find the row number of existing entry if it exists
        let existingRowNumber = null;
        if (data && data.r3Form) {
            const existingEntry = data.r3Form.find(row => row.chataId === selectedChataId);
            if (existingEntry) {
                existingRowNumber = data.r3Form.indexOf(existingEntry) + 2; // +2 because sheet is 1-indexed and has header
            }
        }

        // Prepare form data - matching exact column order (A to J)
        const formData = {
            r3Form: {
                chataId: selectedChataId,                    // Column A
                name: selectedData.name,                     // Column B
                timestamp: new Date().toISOString(),         // Column C
                ascStatus: ascStatus,                        // Column D
                adhdStatus: adhdStatus,                      // Column E
                keyClinicialObservations: window.fieldContents['clinical-observations'] || '',  // Column F
                strengthsAndAbilities: window.fieldContents['strengths-abilities'] || '',       // Column G
                prioritySupportAreas: window.fieldContents['priority-support'] || '',           // Column H
                supportRecommendations: window.fieldContents['support-recommendations'] || '',  // Column I
                professionalReferrals: window.fieldContents.referrals ?                        // Column J
        ...
        if (existingRowNumber) {
            // Update existing row
            submitResponse = await fetch(`${R3_FORM_API}/${existingRowNumber}`, {
                method: 'PUT',
                headers: {
        ...
        // Update cache
        const cachedFormData = getCachedData(CACHE_KEYS.FORM_DATA) || [];
        const filteredCache = cachedFormData.filter(form => form.chataId !== selectedChataId);
        filteredCache.push(formData.r3Form);
        setCacheData(CACHE_KEYS.FORM_DATA, filteredCache);
