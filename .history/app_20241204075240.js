...
async function handleSubmit() {
    try {
        ...
        // Check for existing row number
        const existingRowId = await checkExistingSubmission(selectedChataId);
        if (existingRowId) {
            const proceed = confirm(`This CHATA ID already exists in row ${existingRowId}. Do you want to replace this row?`);
            if (!proceed) {
                return;
            }
        }
        ...
        let response;
        if (existingRowId) {
            // Update existing row using PUT request
            console.log(`Updating row ${existingRowId}`);
            response = await fetch(`${R3_FORM_API}/${existingRowId}`, {
                method: 'PUT',
                headers: {
            ...
            // Create new row using POST request
