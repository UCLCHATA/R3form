...
async function checkExistingSubmission(chataId) {
    ...
    try {
        ...
        if (existingSubmission) {
            ...
            addHeaderWarning(existingSubmission.timestamp);
...
function addHeaderWarning(timestamp) {
    const warning = document.createElement('div');
    warning.className = 'existing-submission-warning';
    warning.innerHTML = `
        <i class="material-icons">warning</i>
        <span>Assessment submitted on ${new Date(timestamp).toLocaleDateString()}</span>
    `;
...
async function handleSubmit() {
    try {
        ...
        if (hasExisting && !confirm('An existing assessment was found for this CHATA ID. Submitting will replace the previous assessment. Do you want to continue?')) {
            return;
        }
        ...
        const cachedFormData = getCachedData(CACHE_KEYS.FORM_DATA) || [];
        const existingIndex = cachedFormData.findIndex(form => form.chataId === selectedChataId);
        if (existingIndex !== -1) {
            cachedFormData[existingIndex] = formData.r3Form;
        } else {
            cachedFormData.push(formData.r3Form);
        }
        setCacheData(CACHE_KEYS.FORM_DATA, cachedFormData);
