...
async function handleSubmit() {
    try {
        ...
        // First, check if an entry exists for this CHATA_ID
        ...
        const existingEntry = data.r3Forms?.find(entry => entry.chataId === selectedChataId);
...
// Let's add a test GET request to verify the endpoint
async function testSheety() {
    try {
        const response = await fetch(R3_FORM_API);
        const data = await response.json();
        console.log('Sheety API test response:', data);
    } catch (error) {
