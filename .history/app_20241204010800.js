...
async function handleSubmit() {
    try {
        ...
        const submitResponse = await fetch(R3_FORM_API, {
            method: 'POST',
            headers: {
        ...
        // Verify the response contains the created entry
        if (!responseData.r3Form) {
            throw new Error('API returned success but no entry was created');
        }
