...
function getChataData() {
  try {
    console.log('Starting getChataData function');
    ...
    if (!chataSheet) {
      console.error('Sheet not found:', chataLookupSheetName);
      return { 'result': 'error', 'error': 'Sheet not found: ' + chataLookupSheetName };
    }
    console.log('Opened sheet:', chataSheet.getName());
    
    // Get all data from columns A and B
    var dataRange = chataSheet.getRange('A:B').getValues();
    console.log('Raw data rows:', dataRange.length);
    ...
    console.error('Error in getChataData:', error);
