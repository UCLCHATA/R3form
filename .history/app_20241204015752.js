...
function checkExistingSubmission(chataId) {
    try {
        ...
        const existing = formData?.find(form => form.chataId === chataId);
        ...
        if (existing) {
            ...
            warning.innerHTML = `
                ...
                <span>Assessment submitted on ${new Date(existing.timestamp).toLocaleDateString()}</span>
            `;
            
            const headerControls = document.querySelector('.header-controls');
            headerControls.appendChild(warning);

            // Also add a warning near the submit button
            ...
            // Remove submit warning if it exists
