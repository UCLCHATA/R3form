...
async function handleSubmit() {
    try {
        ...
        // Prepare the form data matching Sheety's column names
        const formData = {
            r3Form: {
                chataId: selectedChataId,
                childName: selectedData.name,
                timestamp: new Date().toISOString(),
                ascStatus: ascStatus,
                adhdStatus: adhdStatus,
                keyObservations: window.fieldContents['clinical-observations'] || '',
                strengthsAbilities: window.fieldContents['strengths-abilities'] || '',
                prioritySupport: window.fieldContents['priority-support'] || '',
