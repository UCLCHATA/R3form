...
const SHEETY_BASE_URL = 'https://api.sheety.co';
const SHEETY_PROJECT = 'chataScores'; // Your actual project name
const SHEETY_API_KEY = ''; // Will be provided by you
...
async function loadChataData(forceRefresh = false) {
    try {
        ...
        // Construct the URL with proper casing and project name
        const url = `${SHEETY_BASE_URL}/UCL/${SHEETY_PROJECT}/allUrl`;
...
async function handleSubmit() {
    try {
        ...
        // Construct the URL with proper casing
        const url = `${SHEETY_BASE_URL}/UCL/${SHEETY_PROJECT}/r3Form`;
        const headers = {
            'Accept': 'application/json',
            'Content-Type': 'application/json'
        };
        ...
        const existingEntriesResponse = await fetch(`${url}?filter[chataId]=${selectedChataId}`, {
            method: 'GET',
            headers: headers
        });
