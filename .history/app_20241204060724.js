    ...
    document.querySelectorAll('.expandable-section').forEach(section => {
        ...
        section.addEventListener('dblclick', (e) => {
            ...
            if (textarea.classList.contains('expanded')) {
                ...
                textarea.dataset.originalPosition = textarea.style.position;
            } else {
                // Restore original state
                textarea.style.height = textarea.dataset.originalHeight || '82px';
        ...
        backdrop.addEventListener('click', () => {
            ...
            if (expandedTextarea) {
                ...
                expandedTextarea.style.height = expandedTextarea.dataset.originalHeight || '82px';
                backdrop.classList.remove('active');
            }
        });
...
function removeEventListeners() {
    ...
    const chataIdSelect = document.getElementById('chata-id-select');
    if (chataIdSelect) {
        const newSelect = chataIdSelect.cloneNode(true);
        chataIdSelect.parentNode.replaceChild(newSelect, chataIdSelect);
    }
...
function clearForm() {
    ...
    // Reset expandable fields to default height
    document.querySelectorAll('.field-preview').forEach(field => {
        field.style.height = '82px';
    });

    // Clear any stored cache for the current CHATA ID
    clearCurrentCache();
}
...
// Function to clear current cache
function clearCurrentCache() {
    const chataIdSelect = document.getElementById('chata-id-select');
    if (chataIdSelect && chataIdSelect.value) {
        localStorage.removeItem(`formData_${chataIdSelect.value}`);
        localStorage.removeItem(`pdfUrl_${chataIdSelect.value}`);
    }
}
