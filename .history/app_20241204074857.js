...
async function handleSubmit() {
    try {
        ...
        if (hasExisting) {
            ...
            const existingData = await fetch(R3_FORM_API);
            const existingJson = await existingData.json();
            const existingRow = existingJson.r3Form.find(row => row.chataId === selectedChataId);
            
            if (!existingRow || !existingRow.id) {
                throw new Error('Could not find existing submission row ID');
            }

            // Update existing submission using PUT request
            console.log(`Updating existing submission with row ID: ${existingRow.id}`);
            response = await fetch(`${R3_FORM_API}/${existingRow.id}`, {
                method: 'PUT',
                headers: {
        ...
        const cachedFormData = getCachedData(CACHE_KEYS.FORM_DATA) || [];
        const updatedFormData = hasExisting 
            ? cachedFormData.map(form => form.chataId === selectedChataId ? formData.r3Form : form)
            : [...cachedFormData, formData.r3Form];
        setCacheData(CACHE_KEYS.FORM_DATA, updatedFormData);

        alert(hasExisting ? 'Assessment updated successfully!' : 'Assessment submitted successfully!');
