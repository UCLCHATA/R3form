...
// Function to handle focus overlay
function handleFocusOverlay(show = true) {
    const overlay = document.querySelector('.focus-overlay');
    if (show) {
        overlay.classList.add('active');
        document.querySelector('.form-content').classList.add('muted');
    } else {
        overlay.classList.remove('active');
        document.querySelector('.form-content').classList.remove('muted');
    }
}

// Function to handle CHATA ID selection
function handleChataIdChange(event) {
    const selectedId = event.target.value;
    if (selectedId) {
        handleFocusOverlay(false);  // Remove overlay when CHATA ID is selected
        ...
        handleFocusOverlay(true);  // Show overlay when no CHATA ID is selected
    }
}
...
function clearForm() {
    // ... existing clear form code ...
    handleFocusOverlay(true);  // Show overlay after form clear
    document.getElementById('chata-id-select').value = '';  // Reset dropdown
}

// Initialize page
document.addEventListener('DOMContentLoaded', async () => {
    try {
        handleFocusOverlay(true);  // Show overlay on initial load
        await loadAllData();
        setupEventListeners();
        setupModalHandlers();
