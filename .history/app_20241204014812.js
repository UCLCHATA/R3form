...
// Function to check if we have sufficient cached data
function hasSufficientCachedData() {
    try {
        const chataData = getCachedData(CACHE_KEYS.CHATA_DATA);
        const formData = getCachedData(CACHE_KEYS.FORM_DATA);
        const pdfUrls = getCachedData(CACHE_KEYS.PDF_URLS);

        // Check if we have enough CHATA data
        const hasChataData = chataData && Array.isArray(chataData) && chataData.length >= 2;
        ...
        console.log('Cache status:', {
            ...
            hasPdfUrls: !!pdfUrls,
            chataDataCount: chataData?.length || 0
        });
...
async function loadAllData(forceRefresh = false) {
    try {
        ...
        if (!forceRefresh && isCacheValid(CACHE_KEYS.CHATA_DATA)) {
            ...
            const cachedPdfUrls = getCachedData(CACHE_KEYS.PDF_URLS);

            if (cachedChataData && cachedChataData.length > 0) {
        ...
        fetchSubmittedForms().then(formData => {
            setCacheData(CACHE_KEYS.FORM_DATA, formData);
        });
        ...
        if (cachedData && cachedData.length > 0) {
            ...
            return {
                ...
                pdfUrls: getCachedData(CACHE_KEYS.PDF_URLS) || []
            };
        }
        throw error;
    }
