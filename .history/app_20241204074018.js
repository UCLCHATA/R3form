...
async function handleSubmit() {
    try {
        ...
        if (data && data.r3Form) {
            existingEntry = data.r3Form.find(row => row.chataId === selectedChataId);
            if (existingEntry) {
                if (!confirm('An existing assessment was found for this CHATA ID. Submitting will replace the previous assessment. Do you want to continue?')) {
                    return;
                }
            }
        }
        ...
        const formData = {
            r3Form: {
                ...
                Professional_Referrals: window.fieldContents.referrals ? 
        ...
        let submitResponse;
        if (existingEntry) {
            // Get the row number (add 2 for header row and 1-based indexing)
            const rowIndex = data.r3Form.indexOf(existingEntry) + 2;
            
            // Update existing row
            submitResponse = await fetch(`${R3_FORM_API}/${rowIndex}`, {
                method: 'PUT',
                headers: {
