...
// Function to populate CHATA dropdown
function populateChataDropdown() {
    const select = document.getElementById('chata-id-select');
    if (!select) {
        console.error('CHATA ID select element not found');
        return;
    }
    ...
    defaultOption.value = '';
    defaultOption.textContent = 'Select CHATA ID';
    select.appendChild(defaultOption);

    // Add options from chataData
    if (chataData && Array.isArray(chataData)) {
        chataData.forEach(item => {
            const option = document.createElement('option');
            option.value = item.id;
            option.textContent = `${item.id} - ${item.name}`;
            select.appendChild(option);
        });
        console.log('Populated dropdown with options:', select.innerHTML);
    } else {
        console.error('No CHATA data available for dropdown');
    }
}

// Function to load and populate CHATA data
async function loadAndPopulateChataData() {
    try {
        ...
        // Only populate dropdown after data is loaded
        if (chataData && Array.isArray(chataData)) {
            populateChataDropdown();
        } else {
            throw new Error('CHATA data not properly loaded');
        }
    } catch (error) {
        console.error('Error loading CHATA data:', error);
        throw new Error('Failed to load CHATA data');
    }
}
