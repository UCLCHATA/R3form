        ...
        if (preview) {
            ...
            preview.addEventListener('dblclick', () => {
                ...
                const resizeObserver = new ResizeObserver(() => {
                    const section = preview.closest('.expandable-section');
                    if (section) {
                        const previewHeight = preview.offsetHeight;
                        const headerHeight = section.querySelector('h4')?.offsetHeight || 0;
                        const padding = 24; // Account for section padding
                        section.style.height = `${previewHeight + headerHeight + padding}px`;
                    }
                });
                ...
                preview.addEventListener('input', () => {
                    ...
                    preview.style.height = `${Math.min(scrollHeight, maxHeight)}px`;
                });
                
                const inputHandler = debounce(() => {
                    window.fieldContents[fieldId] = preview.value.trim();
                    saveToLocalStorage();
                }, 300);
                
                preview.addEventListener('input', inputHandler);
            ...
            // Prevent single-click expansion
