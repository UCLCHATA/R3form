...
async function handleSubmit() {
    try {
        ...
        // If existing submission found, first delete all entries with this CHATA ID
        if (hasExisting) {
            try {
                // Delete existing entries
                const deleteResponse = await fetch(`${R3_FORM_API}/chataId/${selectedChataId}`, {
                    method: 'DELETE'
                });
                
                if (!deleteResponse.ok) {
                    throw new Error(`Failed to delete existing entries: ${deleteResponse.status}`);
                }
                
                console.log('Successfully deleted existing entries');
            } catch (error) {
                console.error('Error deleting existing entries:', error);
                alert('Failed to delete existing entries. Please try again.');
                return;
            }
        }

        // Prepare and submit new form data
        const formData = {
            r3Form: {
                ...
                professionalReferrals: window.fieldContents.referrals ? 
        ...
        // Update cache
        const cachedFormData = getCachedData(CACHE_KEYS.FORM_DATA) || [];
        // Remove all entries with this CHATA ID
