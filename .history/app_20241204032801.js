...
// Update event listeners for expandable sections
document.querySelectorAll('.section.expandable-section').forEach(section => {
    section.addEventListener('dblclick', function(e) {
        if (e.target.closest('.button-group') || e.target.closest('select')) return;
        
        const fieldId = this.querySelector('.expandable-field').getAttribute('data-field-id');
        const modalBackdrop = document.querySelector('.modal-backdrop');
        const modalContainer = document.querySelector(`.modal-container[data-field-id="${fieldId}"]`);
        const expandableText = modalContainer.querySelector('.expandable-text');
        const fieldPreview = this.querySelector('.field-preview');
        
        modalBackdrop.classList.add('active');
        modalContainer.classList.add('active');
        expandableText.value = fieldPreview.value;
        expandableText.focus();
        
        // Set initial height based on content
        expandableText.style.height = 'auto';
        expandableText.style.height = Math.min(expandableText.scrollHeight, window.innerHeight * 0.8) + 'px';
    });
});

// Update text area auto-resize
document.querySelectorAll('.expandable-text').forEach(textarea => {
